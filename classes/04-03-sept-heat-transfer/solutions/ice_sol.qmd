...
But here we have a transfer of sensible heat energy but it is converted to latent heat through melting (a phase change).
Should we include that in the conservation equation?
I think so, otherwise the equation implies constant heat energy accumulation, which does not follow our conceptual model.
So, let's use this:

rate of energy accumulation = rate of convective heat transfer in - rate of sensible to latent energy conversion through melting
...
We assume that the ice cube is always at the freezing point, and all (sensible) energy transfered from the air to the ice goes to melting, so the accumulation term is zero.
Then: 

rate of sensible to latent energy conversion through melting = rate of convective heat transfer in 
